...
namespace Akka.Event
{
    ...
    public abstract class LogEvent : INoSerializationVerificationNeeded
    {
        ...
        {
            return string.Format("[{0}][{1:yyyy-MM-dd HH:mm:ss.fff}][Thread {2}][{3}] {4}", LogLevel().ToString().Replace("Level", "").ToUpperInvariant(), Timestamp, Thread.ManagedThreadId.ToString().PadLeft(4, '0'), LogSource, Message);
        }
    }
}
